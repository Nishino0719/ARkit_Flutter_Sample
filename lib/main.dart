...
class _MyAppState extends State<MyApp> {
  ...
  void _onPanHandler(List<ARKitNodePanResult> pan) {
    ...
    if (panNode != null) {
      ...
      boxNode.eulerAngles.value = vector.Vector3(old.value.x, newAngleY, old.value.z);
    }
  }
  ...
  void _onRotationHandler(List<ARKitNodeRotationResult> rotation) {
    ...
    if (rotationNode != null) {
      final rotation =
          boxNode.eulerAngles.value + vector.Vector3.all(rotationNode.rotation);
      boxNode.eulerAngles.value = rotation;
    }
  }
}
